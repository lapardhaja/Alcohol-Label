...
\begin{itemize}
    \item \textbf{Multi-pass OCR:} Three Tesseract passes (original, CLAHE+sharpened, binarized) with different PSM modes. Each pass catches text the others miss; results are merged and deduplicated.
    \item \textbf{IoU + fuzzy deduplication:} Overlapping blocks from multiple passes are merged using IoU and fuzzy similarity. The highest-confidence block is kept, so you get the best read without duplicate noise.
    \item \textbf{Fuzzy matching with OCR tolerance:} Brand, class/type, and warning use rapidfuzz with configurable thresholds. OCR confusables (1/l, 0/O) and typos (Mat vs Malt) are handled so minor scan errors do not cause false fails.
    \item \textbf{Government warning spell correction:} pyspellchecker corrects common OCR errors in the warning text before comparison, reducing false needs-review on minor misspellings.
    \item \textbf{Config-driven rules:} Thresholds, patterns, and beverage-type rules live in \texttt{config/rules.yaml}. Tuning does not require code changes.
    \item \textbf{Seven presets:} Test presets for spirits, beer, and wine match the sample images so agents can try the flow immediately.
\end{itemize}
